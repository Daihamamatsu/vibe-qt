...
- **`computeTurtlePlan`（`src/utils/turtle.ts`）**: ブレイク日（BUY シグナル日）と買値を指定すると、ブレイク日以降をシミュレートする
  - 買い増し (P2/P3/P4): 毎日その日の N で目標を再計算し（`直前ユニットの目標ライン + 0.5 × N`。P2 基準 = 初回買値、到達後は到達時の価格でラインを固定）、終値が到達 (≥) した日に買い増し（同日の複数レベル到達を許容）
  - EXIT: 終値 ≤ `最新エントリーの目標ライン − 2N`（ストップロス。買い増し到達で引き上げられ、買い増し未到達時は `買値 − 2N`）、または終値 < DC10（手仕舞いライン下抜け）の初回到達（同日に両方該当すればストップを優先）
...
## 備考（Issue #63: タートル BUY ブレイクの OBV 検証）

- タートル BUY ブレイク（BUY シグナル日）ごとに OBV 条件 ①②③ を検証し、タートルパネル / 情報パネルに表示する。全条件を満たすブレイクの BUY マーカーはチャート上で**金色の三角**で識別（既存の赤三角は非該当ブレイクに維持）
- **検証条件**（`src/utils/obv.ts` の純粋関数。ルックアヘッドしない — ブレイク日の直前まで（当日の OBV 更新を含む直近 20 日 OBV 最高値比較は除く）のデータのみ参照）:
  - ① ブレイク日当日の OBV が**過去 20 日間 OBV 最高値を更新**（直前 20 取引日の OBV 最高値を厳密に超える）
  - ② ブレイク日が**20 取引日以上ぶり**のブレイク日（直前の BUY シグナル日から 20 取引日経過。過去 BUY がなければ満たした扱い）
  - ③ ブレイク日の直前 **5 取引日（当日除く）**以内に OBV 20 日間最高値の更新日がある
  - OBV 20 日間最高値の更新日は「当日 OBV が直前 20 日の OBV 最高値を厳密に超える日」（履歴 20 日未満の日 = 先頭 20 日は対象外）
- **UI**（`Stock.vue`）:
  - タートルパネル: ブレイク日選択時に **OBV 検証行**を表示（①②③ を ✓/✗ で表示、全有効時は金色 ★）
  - 固定情報パネル: BUY シグナル日をホバー時に ①②③ のカラーチップ（＋全有効で ★）を表示
